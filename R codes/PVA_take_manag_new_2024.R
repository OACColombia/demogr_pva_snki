...
for (j in 1:Nclimaterep) {  
  ...
  for (k in 1:Nrep) {         
    ...
    Na <- round(Nstart2024 * MU.prop * 0.8) #80% adults
    Nj <- round(Nstart2024 * MU.prop * 0.2) #20% sub-adults
    N <-  t(rbind(Nj, Na))
...
write.csv(df_results, "df_results2.csv")
...
df_take0 <- df_statusquo %>% 
  filter(take == 0) %>%
  mutate(calendar_year = 2023 + year)  # so year=1 -> 2024, year=31 -> 2054
...
df_take <- df_statusquo %>% 
  filter(take %in% c(0, 200, 500)) %>% 
  mutate(calendar_year = 2024 + (year - 1))  
...
N.mu.pop<- df_results %>% 
  mutate(calendar_year = 2023 + year) %>%
...
N.mu.pop.sum<- N.mu.pop %>%
  filter(calendar_year > 2024) %>% #remove burnin
  group_by(climate_rep,dem_rep, take, management, MU) %>%
  summarize(Ntot_2054 = last(pop),
...
N.mu.sum<- N.mu %>%
  filter(calendar_year > 2024) %>% #remove burnin
  group_by(climate_rep, dem_rep, calendar_year, take, management) %>%
  summarize(Ntot_2054 = last(total_pop),
            lambda_mean = mean(lambda, na.rm = T),
            lambda_geommean = exp(mean(log(lambda), na.rm = T)),
            lambda3yr_mean = mean(lambda3yr, na.rm = T),
            lambda3yr_geommean = exp(mean(log(lambda3yr), na.rm = T))
...
#Plot super-population at time 2054 at different management scenarios
...
size_fig2 <- ggplot(data = N.mu.sum_status, aes(x=factor(take), y = Ntot_2054, fill = factor(take))) +
  geom_point(position = position_jitter(width = 0.03), size = 0.8, shape = 21, alpha = 0.5, fill = "grey40") +
  ...
  scale_fill_brewer(name = "Take value", 
                    type = "seq", 
                    palette = "Reds") +
  labs(x = "Take values", y = "Population size at 2054") +
  ylim(c(0,3000))+
  theme_bw() +
  theme(axis.title = element_text(size = 12),
        axis.text = element_text(size = 10),
...
write.csv(df_removed, "df_removed2.csv")
...
write.csv(df_take_init, "take_init_MU.csv")
...
N.mu_desired <- N.mu %>% 
  mutate(year = calendar_year - 2023) %>% 
  filter(management == "status_quo",
         year %in% desired_years)
